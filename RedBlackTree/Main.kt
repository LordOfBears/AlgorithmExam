fun main() {
    /*
        ...
        []
        a b + 912.76 * c d e ^ e + / - a sin 2 ^ +   - постфиксная строка
        [(a + b) * 912.76 - c / (d ^ e + e) + (sin a) ^ 2]

     */
    val expr = "-123.7 + (12.1 ^ 6 - 14) * 0.16 / (18.3 + 5.9)"
    val pattern = "<регулярное выражение>".toRegex()
    val list = pattern.findAll(expr).map { it.value }.toList()
    println(list)

}

// Задача 1. Разработать регулярное выражение для парсинга
//           инфиксного арифметического выражения
// Задача 2. Реализовать преобразования распарсенного инфиксного выражения
//           в постфиксное выражение
// Задача 3. Собрать все вместе, чтобы получился калькулятор.
